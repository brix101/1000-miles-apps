...
import { updateClusterMutation } from "@/services/cluster.service";
...
export default function ClusterEditModal() {
  const { toUpdate } = useBoundStore((state) => state.cluster);
  const {
    resetState: resetCategoryState,
    setSelectedItems: setSelectedCategory,
  } = useSelectCategoryFacetStore();
  const {
    resetState: resetCustomerState,
    setSelectedItems: setSelectedCustomer,
  } = useSelectCustomerFacetStore();
  const { resetState: resetTagState, setSelectedItems: setSelectedTag } =
  ...
  const isModalOpen = Boolean(toUpdate);
  ...
  useEffect(() => {
    if (toUpdate) {
      setSelectedCategory(toUpdate.categories);
      setSelectedCustomer(toUpdate.customers);
      setSelectedTag(toUpdate.tags);
...
function CategoryView() {
  ...
  const { setToUpdate } = useBoundStore((state) => state.cluster);
...
function NameView() {
  ...
  const form = useForm<UpdateClusterInput>({
  ...
  const { mutate, isLoading } = useMutation({
    mutationFn: updateClusterMutation,
    onSuccess: (response) => {
      ...
      queryClient.setQueriesData([QUERY_CLUSTERS_KEY], (prev: unknown) => {
        ...
        const upatedCluster = clusters.map((cluster) => {
          if (cluster.id === parsedResponse.id) {
  ...
  function onSubmit(values: UpdateClusterInput) {
    mutate({ ...values, name });
  }
      ...
      >
        ...
        <div className="col-12 col-sm-auto">
          <div className="text-center text-sm-start">
            ...
            <p className="text-1100 fs--1">Cluster updated</p>
            <button
              className="btn btn-primary px-6"
              onClick={() => setToUpdate(undefined)}
            >
              Proceed
            </button>
          </div>
